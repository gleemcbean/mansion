...
export class Room {
  ...
  public randomDoorPoint(): DoorPoint {
    return this.shuffledDoorPoints()[0]!;
  }
  ...
  public get area(): number {
    ...
    for (let i = 0; i < n; i++) {
      const [x1, y1] = this.topology[i]!;
      const [x2, y2] = this.topology[(i + 1) % n]!;
      area += x1 * y2 - x2 * y1;
    }
...
export class PositionedRoom extends Room {
  ...
  public translate<T extends CardinalDirection | undefined>(
    ...
    if (direction !== undefined) {
      direction = ((direction! + this.direction) % 4) as T;
  ...
  public override randomDoorPoint(): DoorPoint {
    ...
    const doorPoint = this.t_doorPoints[index]!;
    return doorPoint;
  }
  ...
  public intersects(other: PositionedRoom): boolean {
    ...
    for (let i = 0; i < t_topology.length; i++) {
      let [x1, y1] = t_topology[i]!;
      let [x2, y2] = t_topology[(i + 1) % t_topology.length]!;
